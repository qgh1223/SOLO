...
class SoloTrainTrans(object):
    ...
    def __call__(self, results):
        ...
        regress_ranges = ((512, INF),(256, 512),(128, 256),(64, 128),(-1, 64))
        ...
        for i in range(5):
            h[i], w[i] = img_h/fpn_size[i], img_w/fpn_size[i]
        
        # label assign for each box 
        for i in range(obj_num):
            for j in range(5):
                if regress_ranges[j][0] < long[i] <= regress_ranges[j][1]:
                    left_ = np.int(left[i]//w[j])
                    right_ = np.int(right[i]//w[j])+1
                    top_ = np.int(top[i]//h[j])
                    bottom_ = np.int(bottom[i]//h[j])+1
                    f_ind = j
                    break
            label = labels[i]
            for m in range(top_, bottom_):
                for n in range(left_, right_):
                    category_targets[f_ind][m, n] = label
                    point_ins[f_ind][m, n]=i
